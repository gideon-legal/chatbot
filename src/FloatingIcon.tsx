    ...
    return (
      ...
      >
        {activity && activity.text !== '' && (
          <span
            className={`wc-floating-message ${
              visible && activity != null ? 'visible' : ''
            }`}
          >
            {activity.text}
          </span>
...
export const FloatingIcon = connect(
  (state: ChatState) => {
    return {
      activities: state.history
        ? state.history.activities.filter(
            (activity: Activity & { text: string }) =>
              activity.text !== state.format.strings.pingMessage
          )
        : null,
      format: state.format
    };
  ...
  (stateProps: any, dispatchProps: any, ownProps: any): FloatingIconProps => ({
    ...
    url: stateProps.format.widgetSameAsLogo
      ? stateProps.format.logoUrl
      : stateProps.format.widgetUrl
  })
)(FloatingIconView);
